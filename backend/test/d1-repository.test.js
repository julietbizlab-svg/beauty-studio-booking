...
test("狀態轉換：四種 active/completed 皆為已確認；兩種取消對應客人／業主", async function () {
  ...
  assert.deepEqual(
    bookings.map(function (b) { return b.internalStatus; }),
    ["pending", "confirmed", "checked_in", "completed", "cancelled_by_customer", "cancelled_by_store"]
  );
  bookings.forEach(function (b) {
    assert.ok(!Object.prototype.hasOwnProperty.call(b, "tenant_id"));
    assert.ok(!Object.prototype.hasOwnProperty.call(b, "staff_id"));
  });
  assert.deepEqual(
...
var CUSTOMER_CANCELLABLE_SQL = CUSTOMER_CANCELLABLE_STATUS_SQL;
var STAFF_CANCELLABLE_SQL = STAFF_CANCELLABLE_STATUS_SQL;
...
test("cancelBooking batch 順序為 log INSERT...SELECT → booking UPDATE，且都限 active statuses", async function () {
  ...
  assert.ok(statements[0].sql.includes("b.status IN " + CUSTOMER_CANCELLABLE_SQL));
  assert.ok(statements[1].sql.includes("status IN " + CUSTOMER_CANCELLABLE_SQL));
});
...
test("cancelBookingByOwner 的 SELECT／log／UPDATE 都限制 tenant_id，順序 log→UPDATE", async function () {
  ...
  assert.ok(statements[0].sql.includes("b.status IN " + STAFF_CANCELLABLE_SQL));
  assert.ok(statements[1].sql.includes("status IN " + STAFF_CANCELLABLE_SQL));
});
...
var VISIBLE_STATUS_LIST = CUSTOMER_VISIBLE_STATUS_SQL;
...
test("getTodayBookingsForOwner 回傳完整 DTO，台北時間與取消者轉換正確", async function () {
  ...
  assert.equal(dto.internalStatus, "cancelled_by_store");
...
test("getOwnerBookingsForMonth 計數、跨 UTC 分組、owner DTO 欄位與排序", async function () {
  ...
  var confirmedBooking = day18.bookings.find(function (b) { return b.id === "m2"; });
  assert.equal(confirmedBooking.internalStatus, "confirmed");
  var checkedInBooking = day18.bookings.find(function (b) { return b.id === "m3"; });
  assert.equal(checkedInBooking.internalStatus, "checked_in");
  var cancelledBooking = day18.bookings.find(function (b) { return b.id === "m5"; });
  assert.equal(cancelledBooking.internalStatus, "cancelled_by_customer");
