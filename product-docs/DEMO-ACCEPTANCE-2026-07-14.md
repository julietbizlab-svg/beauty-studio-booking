...
| **長時服務重疊防呆（基礎款）** | ✅ 後端已實作 | 依開始時間＋服務時長建立 busy 區間；`/api/slots`、`/api/slots/month`、建立預約同一規則；首尾相接可約 |
| 建立預約 | ✅ 通過 | 寫入 Notion bookings；重疊時拒絕建立 |
| 我的預約 | ✅ 通過 | 顯示本人預約紀錄 |
| 取消預約 | ✅ 通過 | 狀態更新為已取消 |
...
**產品定位**：客人端月曆選日期、長時服務重疊防呆屬**基礎款**，不是加購。

### 長時服務／重疊規則（驗收說明）

| 情境 | 預期 |
|------|------|
| 服務 120／180 分 | 僅顯示「開始＋時長」整段皆空的開始時間 |
| 已有預約占用中段 | 會與之重疊的開始時間不顯示、建立亦被拒 |
| 10:00–11:00 與 11:00–12:00 | **可**相接預約（不算重疊） |
| 查不到舊預約服務時長 | 後端以保守時長（至少 180 分）占用，避免誤放行 |
...
*文件版本：1.2｜beauty-studio-booking Demo｜不含任何 Token 或密碼*
